...
variable "availability_zones" {
  ...
  default = {
    "eu-west-1a" = { public_cidr = "10.0.0.0/24", private_cidr = "10.0.10.0/24" }
    "eu-west-1b" = { public_cidr = "10.0.1.0/24", private_cidr = "10.0.11.0/24" }
  }
}
...
# CNAME target Vercel gives for dev-admin.de-duke.com -- unique per Vercel
# project, no shared default. Set via TF_VERCEL_CNAME_TARGET.
variable "vercel_cname_target" {
  description = "CNAME target Vercel gives for dev-admin.de-duke.com."
  type        = string
  default     = ""
}
