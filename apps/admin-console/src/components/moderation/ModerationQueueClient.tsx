...
async function submitDecision(
  ...
  const response = await fetch(
    `${API_BASE_URL}/moderation/${listingId}/${action}`,
    {
      method: "POST",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      body?.detail ?? `Failed to ${action} listing (${response.status})`,
    );
  }
}
  ...
  function handleRowResolved(rowKey: string) {
    setItems((prev) => {
      const next = prev.filter(
        (i) => (i.report_id ?? i.listing_id ?? i.created_at) !== rowKey,
      );
  ...
  if (state === "empty") {
    return (
      <p className="text-text-secondary">
        Nothing waiting for review. Queue is clear.
      </p>
    );
  }
    ...
    <>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[820px] border-collapse text-sm">
          ...
          <tbody>
            {items.map((item) => {
              const rowKey =
                item.report_id ?? item.listing_id ?? item.created_at;
                    ...
                    {isReport && (
                      <div className="mt-1 text-xs text-text-secondary">
                        ...
                        {item.reporter_name && (
                          <span>
                            {" "}
                            &middot; reported by {item.reporter_name}
                          </span>
                        )}
                        {item.report_detail && (
                          <p className="mt-1">
                            &ldquo;{item.report_detail}&rdquo;
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    {canDecide ? (
                      <div className="flex gap-sm">
                        <button
                          ...
                          onClick={() =>
                            setPendingDecision({ item, action: "approve" })
                          }
                        >
                          Approve
                        </button>
                        <button
                          ...
                          disabled={rowKey === resolvingId}
                          onClick={() =>
                            setPendingDecision({ item, action: "ban" })
                          }
                        >
                          Ban
                        </button>
      ...
      {pendingDecision && (
        <ModerationDecisionDialog
          ...
          listingTitle={
            pendingDecision.item.title ?? pendingDecision.item.listing_id ?? ""
          }
